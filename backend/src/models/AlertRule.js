...
const conditionSchema = new Schema({
  ...
  active_days: [{ type: String, enum: ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'] }],
...
const alertRuleSchema = new Schema({
  ...
  scope:     { type: String, enum: ['global','caregiver','user','household','device'], default: 'global' },
  condition: { type: conditionSchema, required: true },

  dedupe_strategy: { type: String, enum: ['per_day','per_hour','none'], default: 'per_day' }
...
alertRuleSchema.index({ scope: 1, 'condition.user_id': 1, 'condition.household_id': 1, 'condition.device_id': 1 });
