...
routineSchema.pre('save', function(next) {
  try {
    if (
      Array.isArray(this.occurrences) && this.occurrences.length === 0 &&
      this.device_id && this.expected_start && this.expected_end &&
      Array.isArray(this.days) && this.days.length > 0
    ) {
      this.occurrences = [{
        name: '',
