...
export const getUserById = async (req, res, next) => {
  try {
    ...
    if (req.user?.role === 'cuidador') {
      if (
        user._id.toString() !== req.user.sub &&
        user.caregiver_id?.toString() !== req.user.sub
      ) {
...
export const updateUser = async (req, res, next) => {
  try {
    ...
    if (!Object.keys(updateData).length) {
      return res
        .status(400)
        .json({ error: 'Sin cambios permitidos o válidos' });
    }
    ...
    if (!updatedUser) {
      return res
        .status(404)
        .json({ error: 'Usuario no encontrado o no autorizado' });
    }
