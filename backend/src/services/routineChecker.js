...
function buildWindowForDate(dateLike, startHHMM, endHHMM) {
  const [sh, sm] = String(startHHMM || '08:00')
    .split(':')
    .map(Number);
  const [eh, em] = String(endHHMM || '09:00')
    .split(':')
    .map(Number);
 ...
 */
async function evaluateOccurrence({
  routine,
  occurrence,
  date,
  powerThresholdW = 5,
}) {
  const { windowStart, windowEnd } = buildWindowForDate(
    date,
    occurrence.start,
    occurrence.end
  );
...
export async function getRoutinesStatusForDate(userId, dateLike) {
  ...
  for (const r of routines) {
    const todays = (r.occurrences || []).filter(
      (o) => Array.isArray(o.days) && o.days.includes(dayName)
    );
    for (const occ of todays) {
      const res = await evaluateOccurrence({
        routine: r,
        occurrence: occ,
        date: baseDay,
      });
 ...
 */
export async function evaluateAllForDate(
  dateLike,
  ...
  for (const r of routines) {
    ...
    if (persistAggregate && perRoutine.length > 0) {
      ...
      const aggregatedStatus = hasMissed
        ? 'MISSED'
        : hasComplete
          ? 'COMPLETED'
          : 'PENDING';
      ...
      // Suma de lecturas
      const totalMatched = perRoutine.reduce(
        (acc, x) => acc + (x.matchedEvents || 0),
        0
      );

      // Device “representativo” (el primero que aparezca)
      const firstDev =
        perRoutine.find((x) => (x.occurrence.device_ids || []).length)
          ?.occurrence.device_ids[0] || null;

      // Ventana “agregada” (mínimo inicio, máximo fin)
      const minStart = perRoutine.reduce(
        (d, x) => (!d || x.windowStart < d ? x.windowStart : d),
        null
      );
      const maxEnd = perRoutine.reduce(
        (d, x) => (!d || x.windowEnd > d ? x.windowEnd : d),
        null
      );
      ...
      await RoutineOccurrence.findOneAndUpdate(
        { routineId: r._id, date: baseDay },
        {
          ...
          $set: {
            ...
            windowEnd:
              maxEnd || new Date(baseDay.getTime() + 24 * 60 * 60 * 1000),
