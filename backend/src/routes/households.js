...
router.post('/', async (req, res) => {
  try {
    ...
    if (req.user?.role === 'cuidador' && rawUserIds.length) {
      ...
      for (const u of rawUserIds) {
        if (!allowed.has(String(u))) {
          return res.status(400).json({ error: 'Incluyes pacientes que no son tuyos' })
        }
      }
    }
    ...
    const populated = await Household.findById(h._id).populate('users','name email role').lean()
...
router.get('/', async (req, res) => {
  try {
    ...
    const list = await Household
      .find(filter)
      .populate({ path:'users', select:'name email role', strictPopulate:false })
      .lean()

    res.json(list)
  } catch (e) {
    console.error('GET /households failed:', e)   // << log
    res.status(500).json({ error: e.message || 'No se pudieron listar hogares' })
  }
})
...
router.put('/:id', async (req, res) => {
  try {
    ...
    let nextUsers = Array.isArray(req.body.users) ? req.body.users.map(String) : null
    ...
    if (req.user?.role === 'cuidador') {
      ...
      for (const u of nextUsers) {
        if (!allowed.has(String(u)) && String(u) !== String(prev.owner) && String(u) !== String(req.body.owner || prev.owner)) {
          return res.status(400).json({ error: 'Incluyes pacientes que no son tuyos' })
        }
      }
    }
