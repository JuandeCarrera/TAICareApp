...
router.get('/:id', async (req, res) => {
  ...
  else if (req.user?.role !== 'admin' && req.user?.sub !== id) return res.sendStatus(403)
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (!Object.keys(update).length) return res.status(400).json({ error: 'Sin cambios permitidos' })

    const filter = req.user?.role === 'cuidador'
