...
`;
const Input = styled.input`padding:.5rem;border:1px solid ${({theme})=>theme.colors.border};border-radius:8px;background:${({theme})=>theme.colors.cardBg};color:${({theme})=>theme.colors.text};`;
...
export default function ReglasAlertas() {
  ...
  useEffect(()=>{ load(); }, []);
  async function load(){ setRules(await RulesAPI.list().catch(()=>[])); }

  async function save(){
    if (!form.name.trim()) return alert('Nombre requerido');
    await RulesAPI.create(form);
    setForm({ name:'', enabled:true, kind:'RoutineMissed', type:'routine_missed', severity:'MEDIUM' });
    load();
  }

  async function toggle(r){
    await RulesAPI.update(r._id, { enabled: !r.enabled });
    setRules(rs => rs.map(x => x._id===r._id ? { ...x, enabled: !x.enabled } : x));
  }
      ...
      <Body>
        ...
        <Main>
          ...
          <Card>
            ...
            <Row>
              ...
              <label style={{display:'flex',alignItems:'center',gap:'.5rem'}}>
                <input type="checkbox" checked={form.enabled} onChange={e=>setForm(f=>({...f,enabled:e.target.checked}))}/> Activa
              </label>
          ...
          <Card>
            <h3>Mis reglas</h3>
            {rules.map(r => (
              <div key={r._id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'.5rem 0',borderBottom:'1px dashed var(--border)'}}>
                <div>
                  <strong>{r.name}</strong>{' '}
                  <small>· {r.kind}/{r.type} · {r.severity}</small>{' '}
                  {!r.enabled && <small style={{opacity:.7}}>(desactivada)</small>}
                </div>
                <div style={{display:'flex',gap:'.5rem'}}>
                  <Btn onClick={()=>toggle(r)}>{r.enabled ? 'Desactivar' : 'Activar'}</Btn>
                  <Btn onClick={()=>remove(r._id)}>Eliminar</Btn>
                </div>
              </div>
            ))}
            {!rules.length && <div style={{opacity:.7}}>Aún no tienes reglas.</div>}
          </Card>
        </Main>
      </Body>
      <Footer/>
